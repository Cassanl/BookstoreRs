...
use crate::{error::ApiError, persistence::{self, DBMap}, types::book::Book, AppState};
...
async fn handle(c: Arc<mongodb::Client>, id: String) -> (StatusCode, GetBookResponse) {
    let book: Result<Book, ApiError> = get_book_by_id(c, DBMap { key: "_id", value: id }).await;
...
async fn get_book_by_id<'a>(c: Arc<mongodb::Client>, filter: DBMap<'a, String>) -> Result<Book, ApiError> {
    let book_coll: mongodb::Collection<Book> = c.database(persistence::DB_NAME).collection(persistence::BOOK_COLL);
